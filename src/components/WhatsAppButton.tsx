...
export default function WhatsAppButton({ isBottomBarVisible = false }: WhatsAppButtonProps) {
  ...
  return (
    <aside
      ...
      className={`fixed right-4 sm:right-6 z-50 flex items-center justify-center transition-all duration-300 ease-in-out ${
    ...
    >
      ...
      >
        {/* Subtle pulsing green glow / ring behind the button */}
        <span className="absolute inset-0 rounded-full bg-[#25D366] opacity-60 animate-ping pointer-events-none" style={{ animationDuration: "3s" }} />

        {/* Main WhatsApp Button */}
        <div className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-[#25D366] hover:bg-[#20bd5a] text-white flex items-center justify-center shadow-[0_8px_25px_rgba(37,211,102,0.4)] transition-all duration-300 transform hover:scale-110 active:scale-95 border border-white/20">
          {/* WhatsApp SVG Icon */}
          <svg
